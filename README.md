...
Skill-local scripts follow the [OpenAI skill layout](https://developers.openai.com/plugins/build/skills)
and [Claude plugin layout](https://code.claude.com/docs/en/plugins-reference).
Root-level scripts are useful for shared utilities; this package's writer belongs to Setup.
...
the checked-in definitions. ZIPs go in the ignored `dist/` folder; failed skill checks stop the
